...
\(25\)\IEEEunit{m}, and then letting the \rrtfunnel{}, along with the benchmark
...
experiment will have a crosswind of \(0\) \IEEEunit{m/s}, in order to set up the
experiment baseline. The second experiment will have a crosswind of \(0.3\)
\IEEEunit{m/s}, which is the maximum crosswind handled in the \rrtfunnel{}
...
with a \(0.6\) \IEEEunit{m/s} is performed, to verify the basic assumption that
...
with trunks modeled as circles with a radius of \(0.1\) \IEEEunit{m}, and are
...
The funnel robustness guarantees are only valid if the funnels are composable.
